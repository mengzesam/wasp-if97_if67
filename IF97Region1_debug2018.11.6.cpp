...
double IF97Region1::TH2P(double t,double h,int& itera){
    ...
    if(abs(hh-h)>err){
        double p0=p;
        double h0=hh;
        if((t>=0 && t<=247.5711) || (t>=339.8638 && t<=350)){
            p=ps;
            if(abs(p-p0)<0.2) p=p0+1;
        }else if(t>=285.3041857 && t<339.8638){
            p=ps;
            if(abs(p-p0)<0.2) p=p0+1;        
        }
        pi=p/p_base; 
...
void verify(){
    ...
    //     }
    // }
    double dt=(339.8637-285.3041857)/200;    
    for(t=285.3041857;t<=312.5;t+=dt){
        p=IF97Region4::T2P(t)+0.000000;
        double p1=-7405.88491821289*pow(t/300,6)+42720.3320380859*pow(t/300,5)-102340.687817246*pow(t/300,4)
...
void createFitdata(){
    ...
    for(t=286;t<=308;t+=0.5){
        p=IF97Region4::T2P(t)+0.00001;
        double p1=-7405.88491821289*pow(t/300,6)+42720.3320380859*pow(t/300,5)-102340.687817246*pow(t/300,4)
            +130341.473881092*pow(t/300,3)-93159.5896090619*pow(t/300,2)
            +35800.4528838225*(t/300)-5896.15285930626;
        double dp=(p1-0.5-p)/60;
        while(p<p1-0.2){
            h=IF97Region1::PT2H(p,t);
            double h_=h/1000;
            double t_=t/300;
            cout<<setprecision(10)<<t_*t_<<"\t"<<h_*h_<<"\t"<<t_*h_<<"\t"
                <<t_<<"\t"<<h_<<"\t"<<1<<"\t"<<p<<endl;
            p+=dp;            
        }
    } 
