...
double IF97Region1::HS2P(double h,double s,int& itera){
    double err=ERR;
    double eta=h/3400;
    double sigma=s/7.6;
    double pi=0;
    for(int i=0;i<19;i++)
       pi+=ni_hs2pr1[i]*pow(eta+0.05,Ii_hs2pr1[i])*pow(sigma+0.05,Ji_hs2pr1[i]);
    double p=pi*100;//利用backward function求p初值
    double t=PH2T(p,h);
    double ss=PT2S(p,t);
    itera=0;
    if(abs(ss-s)>err){
